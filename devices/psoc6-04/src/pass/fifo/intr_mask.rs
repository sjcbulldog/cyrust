...
impl R {
    #[doc = "Bit 0 - Mask for corresponding field in INTR register. If CONFIG.CHAIN_TO_NXT is enabled, only FIFO\\[0\\].INTR_MASK.FIFO_LEVEL should be set."]
    #[inline(always)]
    pub fn fifo_level(&self) -> FIFO_LEVEL_R {
        FIFO_LEVEL_R::new((self.bits & 0x01) != 0)
    }
    #[doc = "Bit 1 - Mask for corresponding field in INTR register. If CONFIG.CHAIN_TO_NXT is enabled, only FIFO\\[0\\].INTR_MASK.FIFO_OVERFLOW should be set."]
    #[inline(always)]
    pub fn fifo_overflow(&self) -> FIFO_OVERFLOW_R {
        FIFO_OVERFLOW_R::new(((self.bits >> 1) & 0x01) != 0)
    }
    #[doc = "Bit 2 - Mask for corresponding field in INTR register. If CONFIG.CHAIN_TO_NXT is enabled, only FIFO\\[0\\].INTR_MASK.FIFO_UNDERFLOW should be set."]
    #[inline(always)]
